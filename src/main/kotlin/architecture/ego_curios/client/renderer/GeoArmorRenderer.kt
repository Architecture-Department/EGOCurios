...
open class GeoArmorRenderer<T, S> : IGeoRendererExpand<T, S> where S : IAnimatable<T> {
	...
	override var animatable: S? = null

	override var lastModelInstance: ModelInstance? = null

	override var bufferSource: MultiBufferSource? = null
	override var partialTick: Float = 0f
	...
	fun prepForRender(
		...
		if (currentEntity is AbstractClientPlayer) {
			this.isFineArm = (currentEntity as AbstractClientPlayer).getSkin().model() == PlayerSkin.Model.SLIM
		}
	}
	...
	override fun defaultRender(
		...
		poseStack.pushPose()
		animatable.modelController.model?.run {
			renderModel(origin, pose, poseStack, buffer, packedLight, overlay, color, partialTick)
		}
		layers.forEach {
